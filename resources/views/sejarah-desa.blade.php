    ...
    <div class="card mb-3">
      <div class=" carousel slide carousel-fade" id="controlStyledExample" data-ride="carousel">
        <div class="carousel-indicators">
          @foreach ($sejarah->multipleImage as $i=>$image)
          ...
          <div class="col">
            <div class="d-flex">
              <div class="calendar me-2"><span
                  class="calendar-month">{{$sejarah->created_at->isoFormat('MMM')}}</span><span class="calendar-day">{{
                  $sejarah->created_at->isoFormat('D') }}</span>
              </div>
              <div class="flex-1 fs--1">
                <h5 class="fs-0"><a href="#" class="text-decoration-none text-dark">{{ $sejarah->title }}</a></h5>
                <p class="mb-0">by <a href="#!" class="text-decoration-none">{{ $sejarah->user->name }}</a> &bull; {{
                  $sejarah->created_at->diffForHumans() }} </p>
                <a href="" class="fs-0 text-warning fw-semi-bold">{{ $sejarah->category->nama_category }}</a>
              </div>
            </div>
          </div>
          <div class="col-md-auto mt-4 mt-md-0">
            <button class="btn btn-falcon-default btn-sm me-2" type="button"><span
                class="fas fa-heart text-danger me-1"></span>{{ $sejarah->like }}</button>
